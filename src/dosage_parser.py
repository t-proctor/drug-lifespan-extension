...
MOLARITY_RE = re.compile(
    r"^(?P<value>\d+\.?\d*)\s*(?P<prefix>[pnuµm]?)\s*(?P<unit>M)(?:\s|\b)",  # noqa: E501
    re.IGNORECASE
)
...
DOSAGE_PATTERNS = {
    "ppm": re.compile(
        r"^(?P<value>\d+\.?\d*)\s*ppm(?:\s|\b)", re.IGNORECASE
    ),
    "percent": re.compile(
        r"^(?P<value>\d+\.?\d*)\s*%(?:\s|\b)", re.IGNORECASE
    ),
    "µg_per_ml": re.compile(
        r"^(?P<value>\d+\.?\d*)\s*u?g/ml(?:\s|\b)", re.IGNORECASE
    ),
    "mg_per_ml": re.compile(
        r"^(?P<value>\d+\.?\d*)\s*mg/ml(?:\s|\b)", re.IGNORECASE
    ),
    "mg_per_100ml": re.compile(
        r"^(?P<value>\d+\.?\d*)\s*mg/100\s*m[l|L](?:\s|\b)", re.IGNORECASE
    ),
    "mg_per_kg": re.compile(
        r"^(?P<value>\d+\.?\d*)\s*mg/kg(?:\s|\b)", re.IGNORECASE
    ),
    "mg_per_L": re.compile(
        r"^(?P<value>\d+\.?\d*)\s*mg/l(?:\s|\b)", re.IGNORECASE
    ),
    "µl_per_100ml": re.compile(
        r"^(?P<value>\d+\.?\d*)\s*u?l/100\s*m[l|L](?:\s|\b)", re.IGNORECASE
    ),
    "g_per_L": re.compile(
        r"^(?P<value>\d+\.?\d*)\s*g/l(?:\s|\b)", re.IGNORECASE
    ),
    "µg_per_g": re.compile(
        r"^(?P<value>\d+\.?\d*)\s*u?g/g(?:\s|\b)", re.IGNORECASE
    ),
    "iu": re.compile(
        r"^(?P<value>\d+\.?\d*)\s*iu(?:\s|\b)", re.IGNORECASE
    ),
    "ng_per_ml": re.compile(
        r"^(?P<value>\d+\.?\d*)\s*ng/ml(?:\s|\b)", re.IGNORECASE
    ),
    "mg": re.compile(
        r"^(?P<value>\d+\.?\d*)\s*mg(?:\s|\b)", re.IGNORECASE
    ),
    "µmol_per_L": re.compile(
        r"^(?P<value>\d+\.?\d*)\s*u?mol/L(?:\s|\b)", re.IGNORECASE
    ),
    "g_per_kg": re.compile(
        r"^(?P<value>\d+\.?\d*)\s*g/kg(?:\s|\b)", re.IGNORECASE
    ),
}


def _parse_molarity(s: str) -> tuple[float | None, str | None]:
    """Helper to parse molarity strings. Returns value in M and unit string."""
...
def _parse_other_dosage(s: str) -> tuple[float | None, str | None]:
    """Helper to parse non-molarity dosage strings based on patterns."""
    if not isinstance(s, str):
        return None, None
    try:
        # Normalize: replace mu, strip, remove parenthetical footnotes
...
def parse_dosage_column(df: pd.DataFrame, col_name: str) -> pd.DataFrame:
    ...
    def parse_single_dosage(s):
        ...
        if pd.isna(s) or (
            ...
            (not s.strip() or s.strip().lower() == 'nan')
        ):
    ...
    df_parsed[[DOSAGE_VALUE_COL, DOSAGE_UNIT_COL]] = pd.DataFrame(
        parsed_results.tolist(), index=df_parsed.index
    )
    ...
    # Identify errors (value is NaN but unit != 'missing' or 'unknown')
    ...
    print(
        f" -> Explicitly missing (original NaN): {missing_explicit_count} "
        f"({missing_explicit_count/total_entries*100:.2f}%)"
    )
    print(
        f" -> Successfully parsed (value extracted): {parsed_count} "
        f"({parsed_count/total_entries*100:.2f}%)"
    )
    print(
        f" -> Unmatched/Unknown format: {unknown_count} "
        f"({unknown_count/total_entries*100:.2f}%)"
    )
    if error_count > 0:
        print(
            f" -> Errors during parsing/extraction: {error_count} "
            f"({error_count/total_entries*100:.2f}%)"
        )

    print("\n--- Dosage Unit Distribution (Successfully Parsed) ---")
    # Exclude 'missing', 'unknown', and errors from value counts
    unit_counts = df_parsed.loc[parsed_ok_mask, DOSAGE_UNIT_COL]
    unit_counts = unit_counts.value_counts(normalize=True)
