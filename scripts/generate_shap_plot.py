...
def main():
    ...
    parser.add_argument(
        ...
        help=(f"Path to save the SHAP summary plot (default: "
              f"{os.path.join(DEFAULT_MODEL_DIR, DEFAULT_PLOT_NAME)})")
    )
    ...
    try:
        ...
        print(
            f"Processed test data shape for SHAP: {X_test_processed.shape}"
        )

        # Get feature names after preprocessing
        feature_names_out = preprocessor.get_feature_names_out()
        print(
            f"Retrieved {len(feature_names_out)} feature names after "
            "preprocessing."
        )
        ...
        if (feature_names_out is not None and
            ...
            print(
                "Warning: Feature names mismatch or unavailable. "
        ...
        print(
            f"Error during preprocessing or feature name retrieval: {e}",
            file=sys.stderr
        )
        sys.exit(1)
    ...
    try:
        ...
        print(
            f"Error during SHAP calculation or plotting: {e}", file=sys.stderr)
        sys.exit(1)
