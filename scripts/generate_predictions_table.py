...
DEFAULT_MODEL_DIR = 'models/drug_age_w_descriptors_tuned'
...
def load_data(filepath: str) -> tuple[pd.DataFrame, pd.Series] | None:
    ...
    try:
        ...
        if TARGET_COL not in df.columns:
            print(
                f"Error: Target column '{TARGET_COL}' "
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Load a trained model and generate a table "
            "comparing predictions to actual values."
        )
    )
    parser.add_argument(
        '--input_path', type=str, default=DEFAULT_DATA_PATH,
        help=f"Path to the processed input PKL file (default: "
             f"{DEFAULT_DATA_PATH})"
    )
    parser.add_argument(
        '--model_path', type=str,
        default=os.path.join(DEFAULT_MODEL_DIR, DEFAULT_MODEL_NAME),
        help=(f"Path to the trained model joblib file (default: "
              f"{os.path.join(DEFAULT_MODEL_DIR, DEFAULT_MODEL_NAME)})")
    )
    parser.add_argument(
    ...
    print(
        f"Splitting data using test_size={TEST_SIZE} "
    ...
    try:
        ...
        # target encoder during training could cause issues if unknown
        # categories exist.
        # The pipeline *should* handle this if configured correctly, but good
        # to note.
        sys.exit(1)
    ...
    # Check for compound column
    if 'compound_name' in X_test.columns:
        compound_col = 'compound_name'
    else:
        compound_col = None

    # Check for species column
    species_col = 'species' if 'species' in X_test.columns else None

    if not compound_col:
        print("Warning: 'compound_name' not found. Excluding it from table.")
    if not species_col:
        print("Warning: 'species' column not found. Excluding it from table.")
    ...
    column_order = [
        'Actual Lifespan Change (%)',
        'Predicted Lifespan Change (%)'
    ]

    if species_col:
        results_data['Species'] = X_test[species_col]
        column_order.insert(0, 'Species')  # Add species to the beginning

    if compound_col:
        results_data['Compound'] = X_test[compound_col]
        # If species was added, compound goes before it
        # Otherwise, compound goes to the very beginning
        insert_pos = 0 if not species_col else column_order.index('Species')
        column_order.insert(insert_pos, 'Compound')

    results_df = pd.DataFrame(results_data)[column_order]

    # Round the numeric columns for display
    results_df['Actual Lifespan Change (%)'] = results_df[
        'Actual Lifespan Change (%)'
    ].round(2)
    results_df['Predicted Lifespan Change (%)'] = results_df[
        'Predicted Lifespan Change (%)'
    ].round(2)
    ...
    if n_samples_to_show > 0:
        sample_results = results_df.sample(
            n=n_samples_to_show, random_state=RANDOM_STATE
        )
        print(f"Example Predictions vs Actuals (Random Sample - "
              f"{n_samples_to_show} rows):")
        # Ensure tabulate is installed for to_markdown
        try:
            print(sample_results.to_markdown(index=False))
        except ImportError:
            print("Error: 'tabulate' library not found. Cannot print "
                  "Markdown table.")
